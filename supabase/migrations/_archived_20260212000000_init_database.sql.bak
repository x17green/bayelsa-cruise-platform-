-- Enable pgcrypto for gen_random_uuid() (PostgreSQL 13+ has it built-in)
...
CREATE INDEX idx_boat_operators_user_id ON boat_operators(user_id);
CREATE INDEX idx_boats_operator_id ON boats(operator_id);
CREATE INDEX idx_trips_operator_id ON trips(operator_id);
CREATE INDEX idx_trips_boat_id ON trips(boat_id);
...
ALTER TABLE boat_operators ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Trips are readable by all authenticated users" ON trips
  FOR SELECT USING (auth.role() = 'authenticated');

-- RLS Policies for routes (public read)
CREATE POLICY "Routes are readable by all authenticated users" ON routes
  FOR SELECT USING (auth.role() = 'authenticated');

-- RLS Policies for boats (public read)
CREATE POLICY "Boats are readable by all authenticated users" ON boats
  FOR SELECT USING (auth.role() = 'authenticated');

-- RLS Policies for boat_operators (public read)
CREATE POLICY "Boat operators are readable by all authenticated users" ON boat_operators
  FOR SELECT USING (auth.role() = 'authenticated');
